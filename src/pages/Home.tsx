...
const SupplierCard = ({
  ...
  return <Card className="glass-morphism border-white/10 card-hover overflow-hidden h-full transition-all duration-300 w-full max-w-full" onMouseEnter={() => setIsHovering(true)} onMouseLeave={() => setIsHovering(false)}>
      ...
      height: isMobile ? '130px' : '180px'
        ...
        <img src={supplier.images && supplier.images.length > 0 ? supplier.images[0] : 'https://images.unsplash.com/photo-1581091226825-a6a2a5aee158'} alt={supplier.name} className="w-full h-full object-cover transition-transform duration-500" style={{
        transform: isHovering ? 'scale(1.05)' : 'scale(1)'
      }} onError={e => {
        (e.target as HTMLImageElement).src = 'https://images.unsplash.com/photo-1581091226825-a6a2a5aee158';
      }} />
        <div className="absolute top-2 right-2 flex gap-2">
          {supplier.featured && <Badge className="bg-[#F97316]/90 text-white border-0">
      ...
      <CardContent className="p-4">
        ...
        <div className="flex gap-2 mb-2 flex-wrap">
          {supplier.categories && supplier.categories.slice(0, 2).map(categoryId => <Badge key={categoryId} variant="secondary" className="bg-white/10">
              {getCategoryNameFromId(categoryId)}
            </Badge>)}
        </div>
        <div className="flex justify-between items-center">
          <span className="text-sm text-gray-400">{supplier.city}, {supplier.state}</span>
          <Link to={`/suppliers/${supplier.id}`} className="text-[#9b87f5] hover:text-[#D946EF] text-sm font-medium transition-colors flex items-center gap-1">
            Ver detalhes
...
const ArticleCard = ({
  ...
  return <Card className="glass-morphism border-white/10 card-hover overflow-hidden h-full transition-all duration-300 w-full max-w-full">
      ...
      height: isMobile ? '130px' : '160px'
        ...
        <img src={article.image_url} alt={article.title} className="w-full h-full object-cover transition-transform duration-500 hover:scale-105" onError={e => {
      ...
      <CardContent className="p-4">
        ...
        <div className="flex justify-between items-center">
          ...
          <Link to={`/articles/${article.id}`} className="text-[#9b87f5] hover:text-[#D946EF] text-sm font-medium transition-colors flex items-center gap-1">
            Ler mais
            <ArrowRight className="h-4 w-4" />
          </Link>
        </div>
...
const SkeletonCard = () => {
  const isMobile = useIsMobile();
  return <Card className="glass-morphism border-white/10 overflow-hidden h-full w-full max-w-full">
...
export default function Home() {
  ...
  return <AppLayout>
      ...
      <section className="mb-8 px-4">
        ...
        {loadingSuppliers ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-1 md:gap-1 w-full max-w-full overflow-x-auto">
        ...
        {loadingSuppliers ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
            {[1, 2, 3].map(i => <SkeletonCard key={`popular-skeleton-${i}`} />)}
      ...
      <section className="mb-8 px-4">
        ...
        {loadingArticles ? <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
