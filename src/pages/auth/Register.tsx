...
const FormSchema = z.object({
  ...
  phone: z.string().min(10, { message: "Telefone inválido. Use (XX) XXXXX-XXXX." }).regex(/^\(\d{2}\) \d{5}-\d{4}$/, { message: "Formato de telefone inválido. Use (XX) XXXXX-XXXX." }),
...
const Register = () => {
  ...
  const handlePhoneChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    form.setValue('phone', maskedValue, { shouldValidate: true });
  };
  ...
  return (
    <div className="flex min-h-screen items-center justify-center px-4 py-12" style={{ backgroundColor: '#6d28d9' /* purple-700 */ }}>
      ...
      <Card className="w-full max-w-lg bg-slate-900 shadow-2xl rounded-xl border-slate-700">
        ...
        <form onSubmit={handleSubmit(onFormSubmit)}>
          <CardContent className="space-y-4 p-6 sm:p-8">
            {apiError && (
              <div className="bg-red-700/30 border border-red-600/50 text-red-200 px-4 py-3 rounded-md text-sm text-center">
                {apiError}
              </div>
            )}
            ...
            <div className="space-y-2">
              ...
              <div className="relative flex items-center">
                ...
                <Input 
                  ...
                  className="pl-10 bg-slate-800 border-slate-700 text-white placeholder:text-slate-500 focus-visible:ring-brand-purple/50" 
                />
              </div>
                      ...
                      <Select onValueChange={field.onChange} defaultValue={field.value}>
                        ...
                        <SelectContent className="bg-slate-800 border-slate-700 text-white">
                          {brazilianStates.map((s) => (
                            <SelectItem key={s.value} value={s.value} className="hover:bg-slate-700 focus:bg-slate-700">
                              {s.label}
                            </SelectItem>
                          ))}
                        </SelectContent>
