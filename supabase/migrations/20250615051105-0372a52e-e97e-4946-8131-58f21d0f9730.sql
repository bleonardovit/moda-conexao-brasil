...
-- Corrigir função otimizada para buscar fornecedores com paginação
DROP FUNCTION IF EXISTS public.get_suppliers_paginated(UUID, INTEGER, INTEGER, TEXT, UUID, TEXT, TEXT, TEXT, BOOLEAN, UUID[]);
...
BEGIN
  ...
  IF p_user_id IS NOT NULL AND NOT is_admin THEN
    SELECT 
      CASE WHEN p.trial_status = 'active' THEN TRUE ELSE FALSE END,
      CASE WHEN p.trial_status = 'expired' THEN TRUE ELSE FALSE END
    ...
    IF is_in_trial THEN
      SELECT COALESCE(ftc.allowed_supplier_ids, '{}')
      INTO allowed_supplier_ids
      FROM free_trial_config ftc
      WHERE ftc.user_id = p_user_id;
    END IF;
  END IF;
  ...
  WITH filtered_suppliers AS (
    ...
    WHERE 
      (is_admin OR s.hidden = FALSE) AND
  ...
  SELECT COUNT(*) INTO total_suppliers FROM filtered_suppliers;
  ...
  WITH supplier_data AS (
    SELECT 
      ...
      CASE 
        WHEN p_user_id IS NULL OR is_admin THEN 0
